...
$page = 'subscribers';
$pagetitle = 'Subscription Subscribers';
include 'head2.php'; 
include 'header3.php';
$services = array();
$query = "SELECT `serviceid` FROM `api_services` WHERE `userid` = '".$_SESSION['userid']."' AND `servicetype` = 'subscription'";
...
if((isset($_SESSION['filter']) && $_SESSION['filter'] == 1)) {
	...
	if((isset($_SESSION['sender']) && $_SESSION['sender'] != '')) {  
		$where .= "`short_code` LIKE '%".$_SESSION['sender']."%' AND ";
	}
	...
	if((isset($_SESSION['sent']) && $_SESSION['sent'] != '')) {  
		$where .= "`status` LIKE '%".$_SESSION['sent']."%' AND ";
	}
	if((isset($_SESSION['start_date']) && $_SESSION['start_date'] != '')) {  
		$where .= "`sub_date` >= '".$_SESSION['start_date']."' AND ";
	}
	if((isset($_SESSION['end_date']) && $_SESSION['end_date'] != '')) {  
		$where .= "`sub_date` <= '".$_SESSION['end_date']."' AND ";
	}
	$where = substr($where,0,-4);
}
$where .= " ORDER BY `sub_date` DESC";
$query = "SELECT COUNT(*) as num FROM ".$tbl_name." ".$where;
$total_pages = mysql_fetch_array(mysql_query($query));
$total_pages = $total_pages[num];

$targetpage = "subscribers.php";
$limit = 25;
$page = $_GET['page'];
...
<div class="wrapper">
	<section class="panel">
		<header class="panel-heading">
			...
			<p class="block text-muted">Subscribers for Subscription Services</p>
		</header>
		<div class="scrollable wrapper m-b-none">
			<h5>Filter Subscribers</h5>
			<form action="subscribers.php" method="POST">
				<label><select name="sender" id="sender" class="form-control input-sm">
					...
					<?php
					$shortcodes = array();
					$query = "SELECT DISTINCT `shortcode` FROM `api_services` WHERE `userid` = '".$_SESSION['userid']."' AND `servicetype` = 'subscription'";
					$result = mysql_query($query);
				...
				<label><select name="serviceid" id="serviceid" class="form-control input-sm">
					...
					<?php
					$services = array();
					$query = "SELECT DISTINCT `serviceid` FROM `api_services` WHERE `userid` = '".$_SESSION['userid']."' AND `servicetype` = 'subscription'";
					$result = mysql_query($query);
					while($row = mysql_fetch_array($result, MYSQL_ASSOC)){ $services[] = $row['serviceid']; }
					foreach($services as $service){
				...
				<input type="submit" class="btn" value="Filter">&nbsp;&nbsp;&nbsp;<?php if(isset($_SESSION['filter']) && $_SESSION['filter'] == 1) { ?><a class="btn btn-link" href="subscribers.php?clear=1">Clear Filters</a><?php } ?>
