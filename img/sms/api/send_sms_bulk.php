...
<content>
	...
	<content>
		...
		foreach($smss_ as $sms){
			...
			$query = "INSERT INTO `api_outgoing_sms_bulk` SET 
			`api_user` = '".$api_user."',
			`api_key` = '".$api_key."',
			...
			if($now >= $timestamp){
				...
				elseif(strlen($sms['sender']) > 0 && (int)$sms['sender'] > 0) { */
					$query = "INSERT INTO sdp.outgoing_sms_bulk (`dest_msisdn`,`text_message`,`sender_name`,`service_id`)";
					$query .= "VALUES ('".$sms['recipient']."','".mysql_real_escape_string(stripslashes($sms['message']))."','".$sms['linkid']."','".$sms['serviceid']."')";
				//}
				mysql_query($query);
				$query2 = "UPDATE `api_outgoing_sms_bulk` SET `sent` = '1' WHERE `id` = '".$sms_id."'";
				mysql_query($query2);
			}
		}
		echo '<?xml version="1.0" encoding="ISO-8859-1"?>
